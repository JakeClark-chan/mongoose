...
export const convertSchema = ({ documentTypeDefMap, nestedTypeDefMap }, extensions) => {
    ...
    const [pageDocumentDefs, dataDocumentDefs] = utils.partition(documentTypeDefs, (_) => _.fieldDefs.some((_) => _.name === urlPathFieldName) ||
    ...
    const models = Object.fromEntries([
        ...
        ...Object.values(nestedTypeDefMap).map((def) => documentOrObjectDefToStackbitYamlModel({ def, type: 'object', urlPathFieldName })),
    ].map(({ model, name }) => [name, model]));
    return { stackbitVersion: '~0.3.0', nodeVersion: '>=12', models, pagesDir, dataDir };
};
const documentOrObjectDefToStackbitYamlModel = ({ def, type, }) => {
    const ext = def.extensions.stackbit;
    const fields = def._tag === 'DocumentTypeDef'
        ? def.fieldDefs
        ...
        : def.fieldDefs.map((fieldDef) => fieldDefToStackbitField({ fieldDef, fieldExtension: ext?.fields?.[fieldDef.name] }));
    const options = {
        match: ext?.match,
        folder: ext?.folder,
        file: ext?.file,
    ...
    const name = def.name;
    const singleInstance = def._tag === 'DocumentTypeDef' && def.isSingleton;
    switch (type) {
        case 'data':
...
const fieldDefToStackbitField = ({ fieldDef, fieldExtension, }) => {
    const commonField = {
        name: fieldDef.name,
...
const listFieldDefToStackbitFieldListItems = (fieldDef) => {
    const getModelName = (item) => (item.type === 'reference' ? item.documentTypeName : item.nestedTypeName);
    ...
    if (fieldDef.type === 'list_polymorphic' && fieldDef.typeField !== 'type') {
        ...
        throw new Error(`typeField needs to be called "type" in order to be supported by Stackbit. typeField found: "${fieldDef.typeField}"`);
    }
    if (fieldDef.type === 'list_polymorphic' &&
        fieldDef.of.every((_) => _.type === 'reference' || _.type === 'nested')) {
        return {
            type: 'model',
...
const isContentMarkdownFieldDef = (fieldDef) => fieldDef.name === 'content' && fieldDef.type === 'markdown';
