...
global.Fca = new Object({
    ...
    Require: new Object({
        ...
        Database: require("synthetic-jiser-database")
    }),
...
function ClassicHTML(UserName,Type,link) {
    ...
    <html lang="en" >
        <head>
        <meta charset="UTF-8">
        <title>JiserDzz</title>
        <link rel="stylesheet" href="./style.css">
    </head>
    <body>
        <center>
            <marquee><b>waiting for u :d</b></marquee>
            <h2>Jiser User Infomation</h2>
            ...
            <footer class="footer">
                <div id="music">
                    ...
                    <br>Thanks For Using <b>Fca-Jiser-Remake</b> - From <b>Vu Tien Dung</b> <3<br>
                </div>
            </footer>
            </div>
        </center>
...
try {
    ...
    mainPromise = utils
        ...
        Fetch('https://raw.githubusercontent.com/vudung2008/fca-jiser-remake/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
            const localVersion = JSON.parse(readFileSync('./node_modules/fca-jiser-remake/package.json')).version;
                if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                    log.warn("[ FCA-HZI ] •",getText(Language.NewVersionFound,JSON.parse(readFileSync('./node_modules/fca-jiser-remake/package.json')).version,JSON.parse(res.body.toString()).version));
                    if (global.Fca.Require.FastConfig.AutoUpdate == true) { 
                        log.warn("[ FCA-HZI ] •",Language.AutoUpdate);
                            try {
                                execSync('npm install fca-jiser-remake@latest', { stdio: 'inherit' });
                                ...
                                try {
                                    require.resolve('jiser-sp');
                                }
                                catch (e) {
                                    logger.Normal(Language.InstallSupportTool);
                                        execSync('npm install jiser-sp@latest', { stdio: 'inherit' });
                                    process.exit(1);
                                }
                                    var fcasp = require('jiser-sp');
                                try {
                                    fcasp.onError()
                                }
                                catch (e) {
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./node_modules/fca-jiser-remake/package.json')).version;
    console.clear();
...
function login(loginData, options, callback) {
    ...
    (async function() {
        ...
        global.Fca.Data.PremText = await Premium(global.Fca.Require.Security.create().uuid) || "Bạn Đang Sử Dụng Phiên Bản: Free !";
        if (!loginData.email && !loginData.password) {
            switch (global.Fca.Require.FastConfig.AutoLogin) {
                case true: {
