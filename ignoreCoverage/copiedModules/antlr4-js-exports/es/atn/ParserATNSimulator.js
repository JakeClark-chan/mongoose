...
const PredPrediction_js_1 = __importDefault(require("../dfa/PredPrediction.js"));
const ATNSimulator_js_1 = __importDefault(require("./ATNSimulator.js"));
...
const SingletonPredictionContext_js_1 = __importDefault(require("../context/SingletonPredictionContext.js"));
...
const BitSet_js_1 = __importDefault(require("../misc/BitSet.js"));
const DoubleDict_js_1 = __importDefault(require("../utils/DoubleDict.js"));
...
class ParserATNSimulator extends ATNSimulator_js_1.default {
    ...
    computeTargetState(dfa, previousD, t) {
        ...
        if (this.debug) {
            ...
            console.log("SLL altSubSets=" + (0, arrayToString_js_1.default)(altSubSets) + /*", previous=" + previousD.configs + */
                ", configs=" + reach + ", predict=" + predictedAlt + ", allSubsetsConflict=" + PredictionMode_js_1.default.allSubsetsConflict(altSubSets) + ", conflictingAlts=" + this.getConflictingAlts(reach));
        }
    ...
    s0, input, startIndex, outerContext) {
        ...
        for (;;) {
            ...
            const altSubSets = PredictionMode_js_1.default.getConflictingAltSubsets(reach);
            if (this.debug) {
                console.log("LL altSubSets=" + altSubSets + ", predict=" + PredictionMode_js_1.default.getUniqueAlt(altSubSets) + ", resolvesToJustOneViableAlt=" + PredictionMode_js_1.default.resolvesToJustOneViableAlt(altSubSets));
            }
    ...
    computeReachSet(closure, t, fullCtx) {
        ...
        for (let i = 0; i < closure.items.length; i++) {
            ...
            for (let j = 0; j < c.state.transitions.length; j++) {
                ...
                if (target !== null) {
                    const cfg = new ATNConfig_js_1.default({
                        state: target
                    }, c);
        ...
        for (let i = 0; i < configs.items.length; i++) {
            ...
            if (lookToEndOfRule && config.state.epsilonOnlyTransitions) {
                ...
                if (nextTokens.contains(Token_js_1.default.EPSILON)) {
                    ...
                    result.add(new ATNConfig_js_1.default({
    ...
    computeStartState(p, ctx, fullCtx) {
        ...
        for (let i = 0; i < p.transitions.length; i++) {
            ...
            const c = new ATNConfig_js_1.default({
                state: target,
                alt: i + 1,
    ...
    applyPrecedenceFilter(configs) {
        ...
        for (let i = 0; i < configs.items.length; i++) {
            ...
            if (updatedContext !== config.semanticContext) {
                configSet.add(new ATNConfig_js_1.default({
            ...
            if (!config.context.isEmpty()) {
                for (let i = 0; i < config.context.length; i++) {
                    if (config.context.getReturnState(i) === PredictionContext_js_1.default.EMPTY_RETURN_STATE) {
                        if (fullCtx) {
                            configs.add(new ATNConfig_js_1.default({
                                state: config.state,
                                context: PredictionContext_js_1.default.EMPTY
                            }, config), this.mergeCache);
                            continue;
                        }
                        else {
    ...
    canDropLoopEntryEdgeInLeftRecursiveRule(config) {
        ...
        if (p.stateType !== ATNState_js_1.default.STAR_LOOP_ENTRY)
            return false;
        if (p.stateType !== ATNState_js_1.default.STAR_LOOP_ENTRY || !p.isPrecedenceDecision || config.context.isEmpty() || config.context.hasEmptyPath())
            return false;
        ...
        for (let i = 0; i < numCtxs; i++) {
            ...
            if (returnStateTarget.stateType === ATNState_js_1.default.BLOCK_END && returnStateTarget.transitions.length === 1 && returnStateTarget.transitions[0].isEpsilon && returnStateTarget.transitions[0].target === p)
                continue;
    ...
    getEpsilonTarget(config, t, collectPredicates, inContext, fullCtx, treatEofAsEpsilon) {
        switch (t.serializationType) {
            ...
            case Transition_js_1.default.EPSILON:
                return new ATNConfig_js_1.default({
                    state: t.target
                }, config);
